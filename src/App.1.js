		...
		return (
			<div>
				...
				<p>
					...
					{'< props와 status의 차이 >'} <br/>
					- props : 컴포넌트로 속성값 전달 <br/>
					- status : 컴포넌트 안에서 값 관리 <br/>
					결론적으로 status는 데이터가 상태 값이 변경되어 클라이언트에 보여주는 경우에는 사용하자!
				</p>
				<br/>
				<br/>
				<strong>3단계: UI state에 대한 최소한의 표현 찾아내기</strong>
				<ul>
					<li>
						필요로 하는 변경 가능한 state의 최소 집합 생각
					</li>
					<li>
						중복배제 원칙 <br/>
						음.. 이게 무슨 의미냐면 만약 TODO 리스트를 만드는데 최소한의 필요로 하는 아이템만 배열에 유지하고 구지 TODO 개수를 표현하는데 별도로 만들지 말고 배열의 길이를 가져오라는 얘기임
					</li>
				</ul>
